function [xPrev,indexQpts]=heuristicPrevPoint_v4(Qpts,Qb,xNew, qBnext,Dist,BigNum,B1,AP,N_p)
...
for line=1:size(Qpts,1)
    for rbt=1:N
        cost(line)=cost(line)+Dist(Qpts(line,rbt),xNew(rbt))';%cost1
        % rbt
        %pause
    end
end
...
if ~isempty(IndexFeasibleTrans)%if there are feasible PTS transitions from xNew to another PTS state
    ...
    %         CostPath(e)=CostOfPTStransition(xNew,statePBA(1:N),Dist);
    ...
    xPrev=Qpts(indexFeasiblePTSTrans(IndexFeasibleTrans(1)),:);
    indexQpts=indexFeasiblePTSTrans(IndexFeasibleTrans(1));
end
% 
% path=indGoal;
% index=indGoal;
