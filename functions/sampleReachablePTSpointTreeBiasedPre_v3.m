function [xRand,distRand]=sampleReachablePTSpointTreeBiasedPre_v3(Qpba,B1,N,T,ind,AP,N_p, finalState,distanceB,SetMinDist)
...
QBNext=NextNBAState(qBrand,satRand,B1);% reachable set of NBA states from qBrand $R_B(q_P^rand)$
...
if ~isempty(QBNext)
    ...
    for q=1:length(Set)
        ...
        if GoalLevel~=0
            goal=find(GoalLevel==distB);%"R_b^feas(q_B^min) without feasiblity"      
        else
            goal=finalState;
        end
    ...
    cand=find(~cellfun('isempty', flagGS));
    %randq=randi(length(cand));%delete the "break" and uncomment it
...
%j
        if isempty(find(j==rbts, 1))
            ...
            if probNew<newThr
                ...
                [cost1,r_path, ~] = graphshortestpath(T.adjList,qPTSrand(j),goalLoc);
                if length(r_path)==1
                    xRand(j) = r_path(1);
                else
                    xRand(j) = r_path(2);
